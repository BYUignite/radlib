import numpy as np
import sys

from cyparallel_planes import parallel_planes
#from parallel_planes import parallel_planes

sys.path.append('/Users/dol4/codes/radlib/installed/lib/python3.8/site-packages')

from pyrad import pyrad_planck_mean as rad_planck_mean
from pyrad import pyrad_wsgg        as rad_wsgg
from pyrad import pyrad_rcslw       as rad_rcslw

################################################################################

#--------------------- parameters

P      = 101325.0
L      = 0.3
nGG    = 24
ntheta = 101
nx     = 1001
Twall  = 500.0
Tmax   = 2500.0
H      = 0.1

nGGa   = nGG+1

xco2=0.0; xco=0.0; xh2o=0.1; xch4=0.0; fvs=0.0

#---------------------

xCO2   = np.full(nx, xco2, dtype=np.float64)
xCO    = np.full(nx, xco,  dtype=np.float64)
xH2O   = np.full(nx, xh2o, dtype=np.float64)
xCH4   = np.full(nx, xch4, dtype=np.float64)
fvsoot = np.full(nx, fvs,  dtype=np.float64)

x = np.zeros(nx)
T = np.full(nx,Twall, dtype=np.float64)

dx = L/(nx-1)
x[0] = 0.0
Tavg = T[0]
for i in range(1,nx):
    x[i] = x[i-1] + dx
    if x[i] <= L/2-H/2 or x[i] >= L/2+H/2:
        T[i] = Twall
    elif x[i] < L/2:
        T[i] = Twall + (Tmax-Twall)/(L/2-(L/2-H/2))*(x[i]-(L/2-H/2))
    else:
        T[i] = Tmax + (Twall-Tmax)/((L/2+H/2)-L/2)*(x[i]-L/2)
    Tavg += T[i]
Tavg /= nx

#rcslw = rad_rcslw(nGG, P, Tmax, xh2o, xco2, xco, fvs)  # CHANGE: use T=Tmax or T=Tavg for plot
rcslw = rad_rcslw(nGG, P, Tavg, xh2o, xco2, xco, fvs)  # CHANGE: use T=Tmax or T=Tavg for plot

#------------------- get q, Q

x, xQ, q, Q = parallel_planes(rcslw, L, ntheta, P, T, xH2O, xCO2, xCO, xCH4, fvsoot)

print("# x (m), Q (kW/m3)")
for i in range(len(xQ)):
    print(f'{xQ[i]:.5f}  {Q[i]/1000:.5f}')

#print("# x (m), q (kW/m2)")
#for i in range(len(x)):
#    print(f'{x[i]:.5f}  {q[i]/1000:.5f}')

    #-------------------------------------------------------------------------

print()

